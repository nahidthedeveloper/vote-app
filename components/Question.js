...
const Question = ({ question, register }) => {
    const { options, id, question_total_votes, option_votes_count } = question
            ...
            <div className="grid grid-cols-6 md:grid-cols-12 gap-4">
                {options.map((option, index) => (
                    <div className="col-span-6" key={index}>
                        <Option option={option} q_id={id} register={register}  question_total_votes={question_total_votes} option_votes_count={option_votes_count}/>
                    </div>
                ))}
            </div>
        </div>
